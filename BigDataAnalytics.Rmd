...
```{r}
a <- c("Attacking Midfield","Central Midfield")
df2 <- df

df2<- df2 %>% mutate(Position = replace(Position,Position == "Central Midfield" a,"Central"))
df2
```
```{r}
...
```
```{r}
df2 <- df2 %>% mutate(Position = ifelse(Position %in% Central, "Central",ifelse(Position %in% Defense,"Defense",ifelse(Position %in% Left,"Left",ifelse(Position %in% Right, "Right", ifelse(Position %in% Hybrides,"Hybrides","Hybrides"))))))
```
